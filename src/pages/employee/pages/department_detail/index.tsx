...
const Index = () => {
  ...
  useEffect(() => {
    if (department) {
      form.setFieldsValue({
        ...
        foundedAt: department.foundedAt
          ? dayjs(department.foundedAt)
          : undefined,
      });
    }
  }, [department, form]);
  ...
  const { mutate: updateDepartment, isPending: isLoadingUpdate } =
    useUpdateDepartment(department?.id || 0, {
      onSuccess: () => {
        refetchDepartment();
        setEditMode(false);
      },
  ...
  const handleUpdate = useCallback(
    (
      ...
      const updateData: UpdateDepartmentRequest = {
        ...
        departmentCode:
          formValues.departmentCode || changeInfoValue.departmentCode,
        description: formValues.description || changeInfoValue.description,
        managerId:
          pendingManagerId !== undefined
            ? pendingManagerId
            : formValues.managerId || changeInfoValue.managerId,
        foundedAt: formValues.foundedAt
          ? dayjs(formValues.foundedAt).toISOString()
          : changeInfoValue.foundedAt
            ? dayjs(changeInfoValue.foundedAt).toISOString()
            : department?.foundedAt,
  ...
  const handleSaveClick = useCallback(() => {
    ...
    if (
      ...
      newManagerId !== department.managerId
    ) {
  ...
  const handleModalConfirm = useCallback(
    (
      action: "CHANGE_POSITION" | "REMOVE_POSITION",
      newPositionId?: number,
      newDepartmentId?: number,
    ) => {
