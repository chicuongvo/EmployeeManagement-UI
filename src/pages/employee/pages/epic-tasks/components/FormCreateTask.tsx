...
import { createTask, updateTask, setTaskAssignments, type Task, TaskStatus, TaskPriority } from "@/apis/task";
...
const FormCreateTask = ({ open, onCancel, task }: FormCreateTaskProps) => {
    ...
    const { epicId, refetchTasks, selectedTask, setSelectedTask, tasks, parentTaskId, setParentTaskId } = useEpicTask();
        ...
        try {
            ...
            if (currentTask) {
                ...
                if (currentTask.parentTaskId) {
                    queryClient.invalidateQueries({ queryKey: ["task", currentTask.parentTaskId] });
                }
            ...
            message.error(currentTask ? "Cập nhật task thất bại!" : "Tạo task thất bại!");
        }
    };
    ...
    return (
        <Modal
            title={currentTask ? "Cập nhật Task" : (parentTaskId ? "Tạo Subtask" : "Tạo Task Mới")}
            open={isModalOpen}
            onCancel={handleClose}
            onOk={handleSubmit}
            confirmLoading={createMutation.isPending || updateMutation.isPending || setAssignmentsMutation.isPending}
            width={600}
