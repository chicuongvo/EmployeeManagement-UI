...
import { createEpic, updateEpic, setEpicExecutors, type Epic, EpicStatus, EpicPriority } from "@/apis/epic";
...
const FormCreateEpic = ({ open, onCancel, epic }: FormCreateEpicProps) => {
    ...
    const { projectId, refetchEpics, selectedEpic, setSelectedEpic } = useProjectDetail();
    ...
    useEffect(() => {
        if (open && currentEpic) {
            form.setFieldsValue({
                ...
                dateRange: currentEpic.startDate && currentEpic.endDate
    ...
    const handleSubmit = async () => {
        try {
            ...
            message.error(currentEpic ? "Cập nhật epic thất bại!" : "Tạo epic thất bại!");
        }
    };
    ...
    return (
        <Modal
            ...
            onOk={handleSubmit}
            confirmLoading={createMutation.isPending || updateMutation.isPending || setExecutorsMutation.isPending}
            width={600}
