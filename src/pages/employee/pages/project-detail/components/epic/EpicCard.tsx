...
const EpicCard = ({ epic }: EpicCardProps) => {
  ...
  const handleCardClick = () => {
    ...
    navigate(`/management/projects/${projectId}/epics/${epic.id}/tasks`);
  };
  ...
  return (
    ...
    >
      {/* Epic Name */}
      <div className="font-semibold mb-2 line-clamp-2">{epic.name}</div>
      ...
      {(epic.startDate || epic.endDate) && (
        <div className="flex items-center gap-1 text-xs text-gray-500 mb-2">
          ...
          <span>
            {epic.startDate
              ? dayjs(epic.startDate).format("DD/MM/YYYY")
              : "..."}
            {" - "}
      ...
      {/* Task Count & Progress */}
      <div className="text-xs text-gray-500 mb-1">{taskCount} tasks</div>
