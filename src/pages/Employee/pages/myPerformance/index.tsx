...
export default function MyPerformancePage() {
  ...
  const [selectedDetail, setSelectedDetail] =
    useState<PerformanceDetail | null>(null);

  // Filter state
  const [filterYear, setFilterYear] = useState<number | undefined>(undefined);

  const { listMyPerformanceActiveKey, setListMyPerformanceActiveKey } =
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [performanceData, criteriaData] = await Promise.all([
        performanceService.getByEmployeeId(
          userProfile.id,
          undefined,
          filterYear,
        ),
    ...
    const actionColumns: ColumnsType<EmployeePerformanceItem> = [
      {
        ...
        render: (_, record) => {
          ...
          return (
            <Tag color={color}>{score != null ? score.toFixed(2) : "-"}</Tag>
          );
        },
      },
      {
  ...
  return (
    <PageContainer
      header={{
        breadcrumb: {
          items: [{ title: "Hồ sơ nhân viên" }, { title: "Đánh giá của tôi" }],
        },
      }}
      title={<PageTitle title="Đánh giá của tôi" />}
    >
